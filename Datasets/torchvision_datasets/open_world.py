...
class OWDetection(VisionDataset):
    ...
    """
    """
    file_names = [imgid] for img in '../data/OWDETR/VOC2007/ImageSets/Main/imgset.txt'
    self.imageset=[imgid] for img in filenames
    self.images=[img] for img in '../data/OWDETR/VOC2007/JPEGImages'
    self.annotations=[anno] for anno in '../data/OWDETR/VOC2007/Annotations'
    self.imgids=[2021+imgid] for img in filenames
    """
    def __init__(self,
                 args,
                 root,
        ...
        for year, image_set in zip(years, image_sets):
            ...
            voc_root = self.root
            # voc_root:'../data/OWDETR/VOC2007'

            # '../data/OWDETR/VOC2007/Annotations'
            annotation_dir = os.path.join(voc_root, 'Annotations')
            # '../data/OWDETR/VOC2007/JPEGImages'
            image_dir = os.path.join(voc_root, 'JPEGImages')

            if not os.path.isdir(voc_root):
                raise RuntimeError('Dataset not found or corrupted.' +
                                   ' You can use download=True to download it')
            # file_names=[000000558840,000000200365,...,000000495357]
            file_names = self.extract_fns(image_set, voc_root)
            self.image_set.extend(file_names)

            # 取出imageset中的每一张图片的文件路径, 放入self.images
            ...
            # imgids=[2021000000558840,2021000000200365,...,2021000000495357]
    ...
    def extract_fns(self, image_set, voc_root):
        ...
        # '../data/OWDETR/VOC2007/ImageSets/Main/t1_train.txt'
